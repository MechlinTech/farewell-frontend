...
const RiderHome = ({ navigation }: any) => {
  ...
  const onToggle = () => {
    ...
    toggleAnim.value = withSpring(newValue ? 1 : 0);
  };

  const dotStyle = useAnimatedStyle(() => ({
    transform: [
      {
        translateX: toggleAnim.value * travelDistance,
      },
    ],
  ...
  const HeaderSection = React.useMemo(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- navigation from tab is stable
    [],
  );
  ...
  const StatusSection = React.useMemo(
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- dotStyle, onToggle are stable
    [isOnline],
  );
  ...
  const BankingSection = React.useMemo(
    () => (
      <Pressable
        ...
        onPress={() => {
          Navigator.pushScreen(navigation, 'AddBankDetails');
        }}
      >
        <View style={{ flex: 1 }}>
          <Text style={styles.bankTitle}>Add Banking Details</Text>

          <Text style={styles.bankDesc}>
            Upload Banking details of your Account to{'\n'}use.
          </Text>
        </View>

        <ImageComponent source={images.warning} style={styles.warningIcon} />
      </Pressable>
    ),
    // eslint-disable-next-line react-hooks/exhaustive-deps -- navigation from tab is stable
    [],
    ...
    () => (
      <View style={styles.earningsCard}>
        ...
        <View style={styles.earningsRow}>
          <ImageComponent source={images.dollar} style={styles.dollarIcon} />
  ...
  const RequestHeaderSection = React.useMemo(
    () => (
      <View style={styles.requestHeader}>
        <Text style={styles.requestTitle}>Available Requests</Text>

        <Text
          style={styles.viewAll}
          onPress={() => {
            Navigator.switchToRiderRootTab(navigation, 'RiderBookings');
          }}
        >
          View all
        </Text>
